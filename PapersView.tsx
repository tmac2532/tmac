...
export default function PapersView({ lang, selectedPaperId, onClearSelectedPaper, setView }: PapersViewProps) {
  ...
  // Open modal if selected from home widget
  useEffect(() => {
    if (selectedPaperId) {
  ...
  return (
    <div className="w-full bg-white font-sans">
      ...
      {/* Hero Banner with Laboratory backdrop */}
        ...
        <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full overflow-hidden relative shadow-2xl animate-fade-in border border-slate-200">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button onClick={handleClose} className="p-1.5 rounded-lg hover:bg-slate-200 transition-colors absolute right-4 cursor-pointer">
                <X className="w-5 h-5 text-slate-500" />
              </button>
            </div>
            <div className="p-6 space-y-5">
              {activePaper.youtubeUrl && activePaper.youtubeUrl.includes('youtube.com') ? (
